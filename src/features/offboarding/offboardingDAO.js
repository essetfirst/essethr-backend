...
class OffboardingDAO {
  static async injectDB(conn) {
    if (!templates) {
      templates = conn.collection("offboarding_templates");
      instances = conn.collection("offboarding_instances");
      await instances.createIndex({ org: 1, employeeId: 1 });
    }
  }

  static async listTemplates(org) {
    return templates.find({ org: String(org) }).sort({ createdOn: -1 }).toArray();
  }
  ...
  static async startInstance(doc) {
    ...
    const tasks = (template?.tasks || doc.tasks || []).map((t, i) => {
      ...
      return {
        ...
        dueDate:
          dueDays != null && !Number.isNaN(dueDays)
