...
class LeaveDAO {
  ...
  static async getLeaveById(leaveId) {
    try {
      const query = { _id: new ObjectId(leaveId.id) };
  ...
  static async addLeave(leaveInfo) {
    ...
    //   const existLeave = await leaves.findOne(query);
      ...
      const status = leaveInfo.status || "pending";
  ...
  static async approveLeaves(filterCriteria = {}) {
    try {
      ...
      const query = {
        _id:
          leaveIds.length > 1
            ? { $in: leaveIds.map((id) => new ObjectID(id)) }
            : new ObjectId(leaveIds[0]),
        status: { $ne: "approved" },
      };
      console.log(query);
      ...
      console.log(lId)
      const approveLeave = await leaves.findOne({ _id: new ObjectId(lId) });
  ...
  static async rejectLeaves(filterCriteria = {}) {
    try {
      const { leaveIds = [] } = filterCriteria;
      if (!leaveIds.length) return { modifiedCount: 0 };
      const query = {
        _id:
          leaveIds.length > 1
            ? { $in: leaveIds.map((id) => new ObjectId(String(id))) }
            : new ObjectId(String(leaveIds[0])),
        status: { $ne: "rejected" },
      };
      return await leaves.updateMany(query, {
        $set: { status: "rejected", approved: false, lastModifiedOn: new Date() },
      });
    } catch (e) {
      console.error(chalk.redBright(`Unable to reject leave records, ${e}`));
      return { server: true, error: e };
    }
  }
  ...
  static async deleteLeave(leaveId) {
    try {
      const query = { _id: new ObjectId(leaveId),status:"pending" };
