...
const PayrollDateDAO = require("../payroll/payrollDateDAO");

let attendances;

function orgIdFilter(orgId) {
  if (!orgId) return { $exists: true };
  const s = String(orgId);
  try {
    return { $in: [s, new ObjectId(s)] };
  } catch {
    return s;
  }
}

function flattenAttendanceRecords(result) {
...
function getRemark(policy, gracePeriodMinutes = 0) {
  const t = Date.now();
  const dayKey = DAYS_OF_WEEK[new Date(t).getDay()];
  const present = policy?.[dayKey] || policy?.[new Date(t).getDay()];
  if (!present?.workHours?.workStartTime || !present?.workHours?.workEndTime) {
    return [t, "on_time"];
  }
  const { workStartTime } = present.workHours;
  const { workEndTime } = present.workHours;
  const dayPrefix = new Date(t).toISOString().slice(0, 10);
  const graceMs = (Number(gracePeriodMinutes) || 0) * 60 * 1000;
  const workStartMs = new Date(`${dayPrefix} ${workStartTime}`).getTime() + graceMs;
  const workEndMs = new Date(`${dayPrefix} ${workEndTime}`).getTime();

  const dates = new Date();
  const minutes =
    dates.getMinutes() <= 9 ? `0${dates.getMinutes()}` : dates.getMinutes();
  const checkinTime = `${dates.getHours() + 3}:${minutes}:${dates.getSeconds()}`;
  const j = new Date(`${dayPrefix} ${checkinTime}`).getTime();

  if (j <= workStartMs) {
    return [t, "present"];
  }
  if (j > workStartMs && j <= workEndMs) {
    return [t, "late"];
  }
  return [t, "absent"];
}

async function resolveRemarkFromPolicy(orgId, gracePeriodMinutes = 0) {
  if (!orgId) return "on_time";
  try {
    const policy = await OrgDAO.getAttendancePolicy(orgId);
    if (policy && typeof policy === "object" && !Array.isArray(policy)) {
      const [, computedRemark] = getRemark(policy, gracePeriodMinutes);
      return computedRemark || "on_time";
    }
  } catch {
    /* use default remark */
  }
...
class AttendanceDAO {
  ...
  static async swipe(attendanceInfo) {
    try {
      ...
      if (!checkin) {
        const remark = await resolveRemarkFromPolicy(orgId);
        updateData = {
          ...updateData,
          employeeId,
          checkin: time,
      ...
      return { action, message, ...(error ? { error } : {}) };
    } catch (e) {
  ...
  static async checkin(attendanceInfo) {
    try {
      ...
      const policy = await OrgDAO.getAttendancePolicy(orgId);
      let checkinTs = typeof time === "number" ? time : Date.now();
      let remark = "on_time";
      if (policy && typeof policy === "object" && !Array.isArray(policy)) {
        try {
          const [, computedRemark] = getRemark(policy, gracePeriodMinutes);
          remark = computedRemark || "on_time";
        } catch {
          remark = "on_time";
        }
      }
      ...
      await attendances.insertOne({
        ...
        date,
        checkin: checkinTs,
        remark: inputRemark || remark,
        device,
        status: "pending",
        metadata: {
          gracePeriodMinutes,
          appliedAt: new Date(checkinTs).toISOString(),
        },
      });
  ...
  static async getAttendances(filterCriteria) {
    try {
      ...
      //           $match: {
      //             _id: "$$id",
      //             org: org ? new ObjectID(org) : { $exists: true },
      //           },
      //         },
      //         {
  ...
  static async importAttendance(fileInfo) {
    try {
      ...
      if (!filename?.path) {
        return { error: "No file uploaded", inserted: 0 };
      }

      const rows = await readXlsxFile(fs.createReadStream(filename.path));
      if (!rows || rows.length < 2) {
        return { error: "File is empty or has no data rows", inserted: 0 };
      }
      ...
      for (let i = 0; i < dataRows.length; i += 1) {
        ...
        const [dateVal, employeeId, , checkinVal, checkoutVal, workedHoursVal, remark, status] = row;
        if (!employeeId && !dateVal) continue;

        if (!employeeId || !dateVal) {
          errors.push({ row: i + 2, error: "date and employeeId are required" });
          continue;
        }

        const date = extractDateString(dateVal);
        const checkin = parseImportTime(date, checkinVal);
        const checkout = parseImportTime(date, checkoutVal);

        let workedHours = Number(workedHoursVal);
        if (!workedHours && checkin && checkout) {
      ...
      if (!records.length) {
        return { error: "No valid rows to import", inserted: 0, errors };
      }

      let inserted = 0;
      for (const rec of records) {
        const result = await attendances.updateOne(
          { employeeId: rec.employeeId, date: rec.date, orgId: rec.orgId },
          { $set: rec },
          { upsert: true },
        );
        if (result.upsertedCount || result.modifiedCount) inserted += 1;
      }
      ...
      console.error(
        chalk.redBright(`Unable to import attendance records, ${e}`),
      );
      return { error: e.message, server: true, inserted: 0 };
    }
  }
...
function parseImportTime(dateStr, value) {
  ...
  const date = extractDateString(dateStr);
  const str = String(value).trim();
  if (/^\d+$/.test(str) && Number(str) > 1e12) return Number(str);
  const parsed = new Date(`${date}T${str.includes(":") ? str : `${str}:00`}`);
...
module.exports.flattenAttendanceRecords = flattenAttendanceRecords;
module.exports.orgIdFilter = orgIdFilter;
