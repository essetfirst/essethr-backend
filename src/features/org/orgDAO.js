...
class OrgDAO {
  static async injectDB(db) {
    try {
        ...
        validator: {
          $jsonSchema: {
            ...
            properties: {
              ...
              companySlug: {
                bsonType: "string",
                description: "shared slug for all branches of a company",
              },
              isMainBranch: {
                bsonType: "bool",
                description: "true for the primary branch of a company",
              },
              createdBy: {
                bsonType: "string",
                description: "admin email that owns this company group",
              },
              updatedBy: {
                bsonType: "string",
              ...
              attendancePolicy: {
                bsonType: "object",
                description: "branch attendance policy",
              },
              phone: {
                bsonType: "string",
      ...
      await orgs.createIndex({ companySlug: 1 });
      BranchService.injectDB(db);
    } catch (e) {
  ...
  static async createOrg(orgInfo) {
    try {
      const { parentOrgId, attendancePolicy, ...rest } = orgInfo;
      let newOrgInfo;

      if (parentOrgId) {
        const prepared = await BranchService.prepareBranchCreate(parentOrgId, rest);
        if (prepared.error) return { error: prepared.error };
        newOrgInfo = {
          ...prepared,
          attendancePolicy: attendancePolicy || prepared.attendancePolicy || DEFAULT_ATTENDANCE_POLICY,
        };
      } else {
        const prepared = await BranchService.prepareCompanyCreate(rest);
        newOrgInfo = {
          ...prepared,
      ...
      const result = await orgs.insertOne(newOrgInfo);
      const created = await orgs.findOne({ _id: result.insertedId });
      return {
        ...result,
        org: created
          ? { ...created, _id: String(created._id) }
          : { _id: String(result.insertedId), ...newOrgInfo },
      };
    } catch (e) {
      if (String(e).startsWith("MongoError: E11000 duplicate key error")) {
        return { error: "An org or branch with the given name already exists." };
      }
  ...
  static async getOrgById(orgId) {
    try {
      let query = { _id: new ObjectId(orgId) };

      /** Related collections store `org` as a hex string (`String(orgId)`), not ObjectId.
       * Plain `$lookup` localField `_id` vs foreignField `org` never matches; use `$expr`. */
      /** Match whether related docs store `org` as hex string or ObjectId. */
      const lookupByOrgString = (from, as) => ({
        $lookup: {
          from,
          let: { orgOid: "$_id" },
          pipeline: [
            {
              $match: {
                $expr: {
                  $eq: [{ $toString: "$org" }, { $toString: "$$orgOid" }],
                },
              },
            },
          ],
      ...
      const pipeline = [
        { $match: query },
        lookupByOrgString("departments", "departments"),
        lookupByOrgString("positions", "positions"),
        lookupByOrgString("employees", "employees"),
        lookupByOrgString("leave_types", "leaveTypes"),
        lookupByOrgString("holidays", "holidays"),
        {
          $project: {
            slug: 1,
            name: 1,
  ...
  static async getBranchesForOrg(orgId) {
    return BranchService.getBranchesForOrg(orgId);
  }
  ...
  static async deleteOrg(orgId) {
    try {
      ...
      if (org.isMainBranch) {
        const companySlug = BranchService.resolveCompanySlug(org);
        const siblingCount = await orgs.countDocuments({ companySlug });
        if (siblingCount > 1) {
          return {
            error: "Cannot delete the main branch while other branches exist.",
          };
        }
      }
  ...
  //   try {
  //     let query = { _id: new ObjectID(id), date: getDateString(date) };
  //     return await orgs.findOne(query);
  //   } catch (e) {
  //     console.error(
