...
class PayslipDAO {
  ...
  static async getPayslips(filterCriteria = {}) {
    try {
      const { employees, employeeId, from, to } = filterCriteria;
      let query = {};
      if (employeeId) query.employeeId = String(employeeId);
      if (employees && Array.isArray(employees)) {
        query = { employeeId: { $in: employees.map(String) } };
      }
  ...
  static async getPayslipById(payslipId) {
    try {
      const { ObjectId } = require("mongodb");
      let query = { _id: new ObjectId(String(payslipId)) };
  ...
  static async updateStatusByPayrollId(payrollId, status) {
    try {
      const { ObjectId } = require("mongodb");
      const id = String(payrollId);
      let oid = null;
      ...
      const query = oid
        ? { $or: [{ payrollId: oid }, { payrollId: id }] }
        : { payrollId: id };
      return await payslips.updateMany(query, { $set: { status } });
    } catch (e) {
      console.error(`Unable to update payslip status for payroll, ${e}`);
      return { error: e };
    }
  }
  ...
  static async deletePayslipById(payslipId) {
    try {
      let query = { _id: new ObjectID(payslipId) };
  ...
  static async deletePayrollPayslips(payrollId) {
    try {
      let query = { payrollId: new ObjectID(payrollId) };
      return await payslips.deleteMany(query);
    } catch (e) {
