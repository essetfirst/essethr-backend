...
class PayrollDAO {
  ...
  static async autoLockStalePayrolls(orgId, lockAfterDays = 7) {
    try {
      const cutoff = new Date();
      cutoff.setDate(cutoff.getDate() - lockAfterDays);

      await payrolls.updateMany(
        {
          org: String(orgId),
          status: "pending",
          locked: { $ne: true },
          $or: [
            { createdOn: { $lte: cutoff.toISOString() } },
            { payDate: { $lte: cutoff } },
          ],
        },
        {
      ...
      console.error(`Unable to auto-lock stale payrolls, ${e}`);
    }
  }
  ...
  static async compareWithPrevious(payrollId, org) {
    try {
      const current = await PayrollDAO.getPayrollById(payrollId);
      if (!current || current.error) return { error: "Payroll not found" };
      if (String(current.org) !== String(org)) return { error: "Access denied" };

      const all = await PayrollDAO.getPayrolls({ org: String(org) });
      const list = all?.error ? [] : all;
      const sorted = [...list].sort(
        (a, b) => new Date(b.createdOn || b.fromDate) - new Date(a.createdOn || a.fromDate),
      );
      const idx = sorted.findIndex((p) => String(p._id) === String(payrollId));
      const previous = idx >= 0 && idx < sorted.length - 1 ? sorted[idx + 1] : null;

      const sumPayslips = (payroll) => {
        const slips = payroll?.payslips || [];
        return {
          count: slips.length,
          gross: slips.reduce((s, p) => s + (Number(p.grossPay) || Number(p.totalPayment) || 0), 0),
          net: slips.reduce((s, p) => s + (Number(p.netPay) || Number(p.total) || 0), 0),
        };
      };

      const currentTotals = sumPayslips(current);
      const previousTotals = previous ? sumPayslips(previous) : null;

      return {
        current: {
          id: String(current._id),
          title: current.title || current.period,
        ...
        previous: previous
          ? {
              ...
              title: previous.title || previous.period,
        ...
        delta: previousTotals
          ? {
              headcount: currentTotals.count - previousTotals.count,
              gross: Number((currentTotals.gross - previousTotals.gross).toFixed(2)),
              net: Number((currentTotals.net - previousTotals.net).toFixed(2)),
              grossPct: previousTotals.gross
                ? Number((((currentTotals.gross - previousTotals.gross) / previousTotals.gross) * 100).toFixed(1))
                : 0,
              netPct: previousTotals.net
                ? Number((((currentTotals.net - previousTotals.net) / previousTotals.net) * 100).toFixed(1))
                : 0,
            }
          : null,
      };
  ...
  static async deletePayroll(payrollId) {
    try {
      ...
      return await payrolls.deleteOne({ _id: new ObjectID(payrollId) });
    } catch (e) {
  ...
  static async lockPayroll(payrollId, userId) {
    try {
      const result = await payrolls.findOneAndUpdate(
        { _id: new ObjectId(payrollId), locked: { $ne: true } },
        { $set: { locked: true, lockedAt: new Date(), lockedBy: String(userId), status: "locked" } },
  ...
  static async finalizePayroll(payrollId, userId) {
    try {
      const result = await payrolls.findOneAndUpdate(
        { _id: new ObjectId(payrollId) },
        { $set: { status: "finalized", finalizedAt: new Date(), finalizedBy: String(userId) } },
        { returnDocument: "after" },
      );
      if (result?.error) return result;

      await PayslipDAO.updateStatusByPayrollId(payrollId, "approved");
  ...
  static async addAdjustment(payrollId, adjustment) {
    try {
      const entry = {
        type: adjustment.type,
        label: adjustment.label,
        amount: Number(adjustment.amount),
        employeeId: adjustment.employeeId ? String(adjustment.employeeId) : null,
        createdOn: new Date(),
      };
      const result = await payrolls.findOneAndUpdate(
        { _id: new ObjectId(payrollId), locked: { $ne: true } },
        { $push: { adjustments: entry }, $set: { updatedOn: new Date() } },
        { returnDocument: "after" },
      );
      return result;
    } catch (e) {
